...
def get_jira_tickets(config):
    ...
    if response.status_code ==200 and response.json()['total'] > 0:
        logging.info("JSON Response {0}: {1} Jira tickets with access request found."
        ...
        for issue in response.json()['issues']:
            ...
            print(user_list)
            post_users_group(issue_keys,user_list,config)
...
def post_users_group(issue_keys,user_list,config):

    host = config['api']['host']+'group/user?groupname='+ config['api']['group']
    logging.info("Starting to add qualified users in portfolio create user permission group.")
    print("Starting to add qualified users in portfolio create user permission group.\n")
...
def comment_issue(cert,issue_key,payload):
    ...
    if response.status_code == 201:
      ...
      logging.error("Rasponse {}. "
                    "Access forbidden to the url for user trying to comment in ticket".format(response.status_code))
      print("Access forbidden to url for user trying to comment in ticket\n")
    elif response.status_code == 400:
      logging.error("Response {0}.Not able to comment in {1}."
                    "Either due to missing required fields,invalid values or user doesn't have add comment permission in the project.".format(response.status_code,issue_key))
      print("Not able to comment in {}."
            "Either due to missing required fields,invalid values or user doesn't have add comment permission in the project...\n".format(issue_key))
    else:
